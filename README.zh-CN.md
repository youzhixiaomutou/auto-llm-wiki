...
只要 URL 直接指向 `/v1/chat/completions` endpoint，也可以使用第三方 OpenAI 兼容提供商。点击 **Test connection** 用当前 URL、key 和 model 检查 endpoint 是否返回 HTTP 2xx。
...
- **自动 git 提交**：开启后，每次变更计划应用后自动将 Wiki 文件夹变更提交到 git。
